...
const postAnswer = async(req, res, next)=>{
    const {doubt_id, description,author_id, author_name,author_photo_url,author_year,author_college} = req.body
    try {
        let myuuid = uuidv4();
        
        parentDocRef = db.doc(`AllDoubts/`+doubt_id);
        const subcollectionRef = parentDocRef.collection('Answer');
        const req_body = { doubt_id, answer_id :  myuuid, description,author_id, author_name,author_photo_url,author_year,author_college,net_votes: 0,created_on : Timestamp.now()}
        subcollectionRef.doc(myuuid).set(req_body)
        .then((docRef) => {
